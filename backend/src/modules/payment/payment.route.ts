...
router.post('/create-intent', validate(createPaymentIntentSchema), auditTrail({ action: 'payment_intent_create', entity: 'Payment' }), PaymentController.createPaymentIntent);
...
router.post('/:id/refund', requirePermissions('write:payments'), validate(refundPaymentSchema), auditTrail({ action: 'payment_refund', entity: 'Payment' }), PaymentController.refundPayment);
